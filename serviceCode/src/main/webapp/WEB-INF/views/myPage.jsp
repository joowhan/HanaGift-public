            ...
            <div class="col-md-12">
                ...
                <div class="tab-content p-t-30" id="myTabContent">
                    ...
                    >
                        ...
                        <div class="mine-container mt-5">
                            <div class="d-flex">
                                <!-- Tab navigation -->
                                <ul class="mine-nav-tabs nav flex-column me-3" id="mine-tab" role="tablist">
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link mine-nav-link active" id="mine-savings-tab" data-bs-toggle="tab" data-bs-target="#mine-savings" type="button" role="tab" aria-controls="mine-savings" aria-selected="true">보유 적금</button>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link mine-nav-link" id="mine-foreign-tab" data-bs-toggle="tab" data-bs-target="#mine-foreign" type="button" role="tab" aria-controls="mine-foreign" aria-selected="false">보유 외화</button>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link mine-nav-link" id="mine-stocks-tab" data-bs-toggle="tab" data-bs-target="#mine-stocks" type="button" role="tab" aria-controls="mine-stocks" aria-selected="false">보유 주식</button>
                                    </li>
                                </ul>

                                <!-- Tab content -->
                                <div class="mine-tab-content tab-content flex-grow-1" id="mine-tabContent">
                                    <!-- 보유 적금 탭 -->
                                    <div class="mine-tab-pane tab-pane fade show active" id="mine-savings" role="tabpanel" aria-labelledby="mine-savings-tab">
                                        <h4 class="section-title">보유 적금</h4>
                                        <div class="mine-savings-details">
                                            <c:forEach var="saving" items="${savings}">
                                                <div class="card shadow-sm mb-3">
                                                    <div class="card-body d-flex align-items-center">
                                                        <img src="${saving.imgUrl}" alt="${saving.productName} 이미지" class="savings-img">
                                                        <div>
                                                            <h5 class="card-title">적금 계좌: <strong>${saving.accountNumber}</strong></h5>
                                                            <p class="product-name">상품명: ${saving.productName}</p>
                                                            <p class="card-text">
                                                                금액: <strong>${saving.amount}원</strong><br>
                                                                이율: <strong>${saving.baseRate}%</strong><br>
                                                                만기일: <strong>${saving.maturityDate}</strong>
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <!-- 보유 외화 탭 -->
                                    <div class="mine-tab-pane tab-pane fade" id="mine-foreign" role="tabpanel" aria-labelledby="mine-foreign-tab">
                                        <h4 class="section-title">보유 외화</h4>
                                        <div class="mine-foreign-details">
                                            <c:forEach var="wallet" items="${foreignWallets}">
                                                <div class="card shadow-sm mb-3">
                                                    <div class="card-body d-flex align-items-center">
                                                        <img src="${wallet.logoImageUrl}" alt="${wallet.currencyCode} 로고" class="foreign-logo">
                                                        <div>
                                                            <h5 class="card-title">${wallet.currencyCode}</h5>
                                                            <p class="card-text">
                                                                잔액: <strong>${wallet.balance}</strong>
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <!-- 보유 주식 탭 -->
                                    <div class="mine-tab-pane tab-pane fade" id="mine-stocks" role="tabpanel" aria-labelledby="mine-stocks-tab">
                                        <h4 class="section-title">보유 주식</h4>
                                        <div class="mine-stocks-details">
                                            <c:forEach var="stock" items="${stockHoldings}">
                                                <div class="card shadow-sm mb-3">
                                                    <div class="card-body d-flex align-items-center">
                                                        <img src="${stock.logoImageUrl}" alt="${stock.productName} 로고" class="stock-logo">
                                                        <div>
                                                            <h5 class="card-title">${stock.productName}</h5>
                                                            <p class="card-text">
                                                                수량: <strong>${stock.quantity}주</strong>
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tabButtons.forEach(button => {
            button.addEventListener('click', () => {
                // 탭 선택에 따른 필터링 로직
                const selectedTab = button.getAttribute('data-bs-target');

                // 모든 탭 내용을 비활성화
                const tabContents = document.querySelectorAll('.mine-tab-pane');
                tabContents.forEach(tabContent => {
                    tabContent.classList.remove('show', 'active');
                });

                // 선택된 탭 내용만 활성화
                const targetContent = document.querySelector(selectedTab);
                if (targetContent) {
        ...
        // 첫 번째 탭의 내용을 자동으로 클릭하여 로드
        document.querySelector('#mine-savings-tab').click();
    });
</script>
